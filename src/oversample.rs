...
//!
//! ```rust
//! use rust_dsp_effects::{Effect, Distortion, Oversampled};
//!
...
//!
//! Uses linear interpolation for upsampling and FIR anti-aliasing for
//! downsampling. Filter state is stored in fixed-size arrays for `no_std`
//! compatibility.
...
impl<const FACTOR: usize, E: Effect> Oversampled<FACTOR, E> {
    ...
    ///
    /// Generates FACTOR samples by linearly interpolating between
    /// the previous input and current input.
    #[inline]
    ...
    ///
    /// Filters all samples and returns the last one (decimation).
    #[inline]
...
// Cutoff is set to prevent aliasing at the decimated rate.
// Normalized to sum to 1.0 for unity DC gain.
//
// Generated using windowed-sinc with Kaiser window (beta=5.0).
...
mod tests {
    ...
    fn inner_access() {
        ...
        assert!((oversampled.inner().drive_db() - 20.0).abs() < 0.01);
    }
    ...
    #[test]
    fn distortion_with_oversampling() {
        // Without oversampling
        let mut dist_raw = Distortion::new(SAMPLE_RATE);
        dist_raw.set_drive_db(30.0);
        dist_raw.set_waveshape(WaveShape::HardClip);
        dist_raw.set_tone_hz(20000.0);
        dist_raw.reset();

        // With 4x oversampling
        let mut dist_os = Distortion::new(SAMPLE_RATE);
        dist_os.set_drive_db(30.0);
        dist_os.set_waveshape(WaveShape::HardClip);
        dist_os.set_tone_hz(20000.0);
        let mut oversampled = Oversampled::<4, _>::new(dist_os, SAMPLE_RATE);

        // Process a simple tone
        let freq = 1000.0;
        let mut sum_raw = 0.0;
        let mut sum_os = 0.0;
        ...
        for i in 0..1000 {
            ...
            let input = (2.0 * core::f32::consts::PI * freq * t).sin() * 0.5;

            sum_raw += dist_raw.process(input).abs();
            sum_os += oversampled.process(input).abs();
        }

        // Both should produce similar overall energy
        let ratio = sum_os / sum_raw;
        assert!(
            ratio > 0.5 && ratio < 2.0,
            "Oversampled output energy should be similar to raw, ratio: {}",
            ratio
        );
    }
